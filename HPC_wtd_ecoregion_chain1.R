...
# Forked from Arielle's HPC_run_model_chunks_chain1.R. Only change vs the
# national_scalar fork: the iNat intensity call site adds
# `+ year_effect[inat_cell100[g]]`, and a new ecoregion-level trend
# deviation block (year_region/sigma_region/year_effect) is appended --
# see model_code_ecoregion_trend.R / model_audit_vs_arielle.md for the
# full diff record. Everything else byte-identical to the national_scalar
# fork.
model_code_ecoregion_trend <- nimbleCode({
  ...
  if (has_SVC) {
    ...
    # Ecoregion-level trend deviation, replacing the CAR year_effect field.
    for (r in 1:nregion) {
...
run_chain_chunk <- function(chain_id,
  ...
  model <- nimbleModel(model_code_ecoregion_trend,
